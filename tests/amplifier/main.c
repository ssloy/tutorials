...
int main(void) {
    ...
    for (uint8_t measurement=0; measurement<11; measurement++) {
        ...
        while (1) {
            ...
            // АЦП analog_val бегает от 0 до 255, при этом ноль ампер соответствует 127.5 АЦП
            // поэтому сначала преобразуем analog_val в значение от -255 до 255, с нулём ампер в нуле:  analog_val*2 - 255
            // затем y измеряется в миллиамперах : [-255,255]/255 * (2.5V / .000185V/mA)
            // 2.5/(255*.000185) = 52.99, округляем до 53
            y = ((((int32_t)analog_val)<<1) - 255)*53L;
            ...
            // тогда x = x + e * (1512*.000125*1000), округлим скобки до 189, получим:
            x = x + e*189L;
            ...
            // пропорциональный коэффициент Kp равен двум, но фиксированная точка требует ещё множителя 1000, поэтому 2000L*e
            u = 2000L*e + x; // микровольты
